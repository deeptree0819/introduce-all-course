...
const InquiryBanner = ({ className }: InquiryBannerProps) => {
  return (
    <aside
      className={cn(
        "flex flex-col items-center desktop:max-w-[280px]",
        className
      )}
    >
      ...
      <div className="mt-6 flex flex-col space-y-4">
        <div className="flex flex-row items-center space-x-3">
          <div className="flex flex-col items-center">
            ...
            <div className="whitespace-pre text-xs font-normal">취업준비생</div>
          </div>
          <div className="speech-bubble-right whitespace-pre-line break-keep rounded-xl bg-[#EDF6FF] p-4 text-xs font-semibold text-slate-500">
            <span className="text-slate-900">부산</span>에서 취업 준비 중인 20대
            후반 남성입니다.
            <br /> <span className="text-slate-900">로봇과 AI 분야</span>에
            관심이 있는데 어디부터 준비해야할지 모르겠어요ㅠㅠ
          </div>
        </div>

        <div className="flex flex-row items-center space-x-3">
          <div className="speech-bubble-left whitespace-pre-line break-keep rounded-xl bg-[#EDF6FF] p-4 text-xs font-semibold text-slate-500">
